...
//	PORTC |= (1 << 4);
	...
	// We must ALWAYS be in stop mode for any register below CTL0
	// except for ECR.
	if ((reg == ECR) || wasRunning) {} else if ((reg < CTL0) && (isRunning())) stop();
...
void MPR121_t::run(){
	...
	wasRunning = false;
	//PORTF &= ~(1 << 5);	// !wasRunning
	setRegister(ECR, ECR_backup | 0xCC); // restore backup to return to run mode
	//PORTF |= (1 << 4);	// running
	//PORTC &= ~(1 << 5);
}
...
bool MPR121_t::reset(){
	...
	// FGCDT is one of the few registers that defaults to a non-zero value - 
	...
	if(getRegister(FGCDT)!=0x24){
		error |= 1<<READBACK_FAIL_BIT;
	} else {
		error &= ~(1<<READBACK_FAIL_BIT);
	}

	if((getRegister(TS2)&0x80)!=0){			// Read OVCF bit.
		error |= 1<<OVERCURRENT_FLAG_BIT;
	} else {
		error &= ~(1<<OVERCURRENT_FLAG_BIT);
	}	
...
/*
	...
	setRegister(FDLPROXT,settings->FDLPROXT);
  	setRegister(DRDT, settings->DRDT);
	setRegister(FGCDC, settings->FGCDC);
*/
	setRegister(FGCDT, settings->FGCDT);
...
bool MPR121_t::isRunning(){
	...
	//PORTC |= (1 << 7);
        ...
        value = getRegister(ECR);   // Return the running status of the chip
                if(value&0x3F){
                        //PORTF |= (1 << 4);
			//PORTC &= ~(1 << 7);
                        return true;
...
int MPR121_t::getRawTouchData(){
	if(!isInited()) return false; // avoid out of bounds behaviour
...
void MPR121_t::setNumDigPins(unsigned char numPins){
	...
	if(isRunning()) stop(); // have to stop to change ECR
